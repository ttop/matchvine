export const BRACKET_SIZES = [4, 8, 16, 32, 64, 128];

// MS Paint-style organized palette: 6 rows of 8
export const COLOR_PALETTE = [
  // Row 1 - Vivid
  '#ef4444', '#f97316', '#eab308', '#22c55e', '#14b8a6', '#3b82f6', '#7c3aed', '#ec4899',
  // Row 2 - Medium
  '#f87171', '#fb923c', '#facc15', '#4ade80', '#2dd4bf', '#60a5fa', '#a78bfa', '#f472b6',
  // Row 3 - Pastel
  '#fecaca', '#fed7aa', '#fef08a', '#bbf7d0', '#99f6e4', '#bfdbfe', '#ddd6fe', '#fbcfe8',
  // Row 4 - Muted/Earth
  '#a16207', '#d2b48c', '#556b2f', '#6b8f71', '#64748b', '#3a6d99', '#9f7aea', '#d4918f',
  // Row 5 - Neutral
  '#ffffff', '#e5e7eb', '#d1d5db', '#f5f0e8', '#fef9ef', '#d1fae5', '#e8e0f0', '#fce7f3',
  // Row 6 - Dark (auto text color flips to white for contrast)
  '#000000', '#1f2937', '#7f1d1d', '#9a3412', '#854d0e', '#14532d', '#1e3a8a', '#581c87',
];

export const TEXT_COLORS = [
  '#1a1a1a',
  '#374151',
  '#6b7280',
  '#ffffff',
  '#1e40af',
  '#991b1b',
  '#166534',
];

export const DEFAULT_FONTS = [
  { name: 'Sans-serif',  family: 'sans-serif' },
  { name: 'Serif',       family: 'serif' },
  { name: 'Monospace',   family: 'monospace' },
  { name: 'Cursive',     family: 'cursive' },
];

export const GOOGLE_FONTS = [
  'Kalam',
  'Merriweather',
  'Oswald',
  'Poppins',
  'Pacifico',
  'Anta',
  'Fredoka One',
  'Righteous',
];

export const CELL_WIDTH  = 200;
export const CELL_HEIGHT = 56;
export const ROUND_GAP   = 60;
export const CELL_GAP    = 8;
export const CHAMP_WIDTH = 220;
export const TOP_PADDING = 20;
export const LABEL_MARGIN = 40;
export const LEFT_PADDING = 50;
export const RIGHT_PADDING = 50;
